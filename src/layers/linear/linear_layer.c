...
void linlay_randomize_weights(LinearLayer* linlay, double stdev)
{
	for (int i = 0; i < linlay->num_linmods; ++i)
	{
		linmod_randomize_weights(linlay->linmods[i], stdev);
	}
}
...
void linlay_forward(LinearLayer* linlay, double* input, double* pre_activation, double* output)
{
	...
	_linear_activation_function(pre_activation, output, linlay->num_linmods, linlay->act_type);
}
...
{
	_linear_activation_deriv(linlay_eval->pre_activation, linlay_eval->output, linlay_grad->grad_out_net, linlay->num_linmods, linlay->act_type);
	for (int i = 0; i < linlay->num_linmods; ++i)
	{
