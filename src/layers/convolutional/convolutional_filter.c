...
void cnn_filteri_grad_compute_weights(ConvolutionalFilterIndexGrad* cnn_filteri_grad, ConvolutionalFilter* cnn_filter, ImageArray* input, ImageArray* grad_loss_prenet)
{
	for (int i = 0; i < cnn_filter->filter_m; ++i)
	{
		for (int j = 0; j < cnn_filter->filter_n; ++j)
		{
			...
			int i_o = cnn_filteri_grad->index_j;
...
{
	cnn_filteri_grad_compute_weights(cnn_filter_grad, cnn_filter, input, grad_loss_prenet);
	cnn_filteri_grad_compute_input(cnn_filter_grad, cnn_filter, grad_loss_prenet);
}
...
ConvolutionalMultiFilter* cnn_mfilter_alloc(int num_filters, int* filter_indexes, int filter_m, int filter_n, int input_m, int input_n, __img_activation_type act_type)
{
	...
	{
		cnn_mfilter->filters[i] = cnn_filter_alloc(filter_m, filter_n);
	}
...
{
	for (int i = 0; i < cnn_fitler->num_filters; ++i)
	{
...
ConvolutionalMultiFilterGrad* cnn_mfilter_grad_alloc(ConvolutionalMultiFilter* cnn_mfilter)
{
	...
	cnn_mfilter_grad->grad_net_input = _mem_alloc(sizeof(ImageArray*) * cnn_mfilter->num_filters);
	...
	for (int i = 0; i < cnn_mfilter->num_filters; ++i)
	{
		...
		cnn_mfilter_grad->grad_net_input[i] = img_array_alloc(cnn_mfilter->input_m, cnn_mfilter->input_n);
...
void cnn_mfilter_grad_free(ConvolutionalMultiFilterGrad* cnn_mfilter_grad)
{
	for (int i = 0; i < cnn_mfilter_grad->num_filters; ++i)
	{
		...
		img_array_free(cnn_mfilter_grad->grad_net_input[i]);
	...
	free(cnn_mfilter_grad->grad_net_input);
...
void cnn_mfilter_grad_compute_bias(ConvolutionalMultiFilterGrad* cnn_mfilter_grad)
{
	...
	img_array_copy(cnn_mfilter_grad->grad_loss_net, cnn_mfilter_grad->grad_loss_bias);
}
